/// AES S-box used for the `sub_bytes` step in AES encryption.
static S_BOX: [u8; 256] = [
...
fn encrypt(state: &mut [u8; 16], round_keys: &[[u8; 16]; 11]) {
...
/// AES inverse S-box used for the `inv_sub_bytes` step in AES decryption.
...
fn inv_mix_columns(state: &mut [u8; 16]) {
    // AES inverse mix columns constants
    fn mul(x: u8, y: u8) -> u8 {
        let mut r = 0u8;
        let mut a = x;
        let mut b = y;
...
fn inv_add_round_key(state: &mut [u8; 16], round_key: &[u8; 16]) {
    // Inverse is identical to add_round_key (XOR is its own inverse)
    for i in 0..16 {
        state[i] ^= round_key[i];
    }
}

fn decrypt(state: &mut [u8; 16], round_keys: &[[u8; 16]; 11]) {
